...
| | Headless `-p` runs load a project's hooks, env and `.mcp.json` without asking (permissions, mcp) | worker adapter and prompt enhancer pass `--strict-mcp-config` and `--setting-sources user`; verified live on 2.1.280 that a repo's MCP canary and SessionStart canary no longer run. `--setting-sources user` also keeps the project's CLAUDE.md out (an earlier note said otherwise; that run had the Read tool on), so the adapter appends the workspace CLAUDE.md with `--append-system-prompt-file` | aligned |
...
| Plugin evals (from `plugin-evals`, not this page) | `claude plugin eval` against a no-plugin baseline whenever rules, skills or hooks change | `plugin/evals/`: two free-grader cases (tooling skill names the Xcode bridge; no pipe-to-shell install). `claude plugin validate` passes. Observed 2026-09-23 on 2.1.280: in the eval sandbox cm does not load (`dependency-unsatisfied`: its required companions are not installed there), and the sandbox does not offer `EnterPlanMode`, so plan-mode cases cannot pass or fail and were removed | gap: evals measure nothing until cm loads in the sandbox (drop the hard dependencies, or a harness option to supply them) |
