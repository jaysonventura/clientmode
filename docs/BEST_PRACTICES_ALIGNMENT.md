...
| Plugin evals (from `plugin-evals`, not this page) | `claude plugin eval` against a no-plugin baseline whenever rules, skills or hooks change | not yet: a run spends model usage (cases x 3 runs x 2 arms, plus judge calls) | gap: suite to be written and its cost approved before it runs |
